...
export default function BudgetTracker() {
  ...
  // Table cell style constants (were missing in original)
  const th = { textAlign: "left", padding: "10px", fontSize: 13, color: "#475569" };
  const td = { padding: "10px", fontSize: 13, color: "#0f172a" };
  ...
  function buildDonut(list) {
    ...
    list.forEach((tx) => {
      ...
      // We'll treat income as positive and expense as positive as well for category sizing.
      // Net is calculated separately in the UI.
      map[tx.category] += Math.abs(Number(tx.amount) || 0);
    });
    const arr = Object.keys(map).map((k) => ({ label: k, value: map[k] }));
    // sort desc to show largest first
  ...
  function exportCSV() {
    ...
    const rows = transactions.map((t) =>
      header
        .map((h) => {
          const v = t[h] === undefined || t[h] === null ? "" : String(t[h]);
  ...
  // Robust CSV parser splitting on commas not inside quotes
  ...
  function importCSVFile(file) {
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (!Array.isArray(data) || data.length === 0) {
          throw new Error("Invalid or empty CSV");
        }
    ...
    reader.onerror = (err) => {
      console.error(err);
      setToastMessage("CSV read error");
    };
    ...
    try {
      ...
      reader.onload = (e) => {
        try {
          ...
          // ensure each entry has an id and numeric amount
          const cleaned = json.map((it) => ({
            id: it.id || uid(),
            date: it.date || new Date().toISOString().slice(0, 10),
            type: it.type || "Expense",
            group: it.group || "Other",
            category: it.category || "Misc",
            description: it.description || "",
            amount: Number(it.amount) || 0,
            account: it.account || DEFAULT_ACCOUNTS[0],
            currency: it.currency || "₹",
          }));
          setTransactions((prev) => [...cleaned, ...prev]);
          setToastMessage("JSON imported successfully!");
        } catch (err) {
          console.error(err);
      ...
      reader.onerror = (err) => {
        console.error(err);
        setToastMessage("JSON read error");
      };
      reader.readAsText(file);
    } catch (err) {
      console.error(err);
    ...
    <div style={{ minHeight: "100vh", padding: "18px", background: "#f7fafc" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
          ...
          <div style={{ fontSize: 12, color: "#475569", background: "#fff", padding: "6px 10px", borderRadius: 999, boxShadow: "0 1px 2px rgba(2,6,23,0.06)" }}>
            Mode: {modeLocal ? "Local" : "Remote"}
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 360px", gap: 18, alignItems: "start" }}>
          <div style={{ background: "#fff", padding: 16, borderRadius: 10, boxShadow: "0 6px 18px rgba(15,23,42,0.04)" }}>
            ...
            <form onSubmit={addTransaction} style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  const c = (categories[g] && categories[g][0]) || "";
                  setForm((prev) => ({ ...prev, group: g, category: c }));
                }}
              ...
              <div style={{ gridColumn: "1 / span 2", display: "flex", gap: 8, alignItems: "center", marginTop: 6 }}>
                <button type="submit" style={{ background: "#2563eb", color: "#fff", border: "none", padding: "10px 14px", borderRadius: 8 }}>
                  Add Transaction
                </button>

                <button type="button" onClick={clearAll} style={{ background: "transparent", color: "#0f172a", border: "1px solid #e2e8f0", padding: "10px 12px", borderRadius: 8 }}>
                  Clear All
                </button>
              </div>
            </form>
            ...
            <div style={{ marginTop: 12, display: "flex", gap: 8, flexWrap: "wrap" }}>
              <button onClick={exportCSV} style={{ background: "transparent", border: "none", color: "#2563eb", fontSize: 13 }}>
                Export CSV
              </button>
              <button onClick={downloadJSONBackup} style={{ background: "transparent", border: "none", color: "#2563eb", fontSize: 13 }}>
                Export JSON
              </button>
              <button onClick={() => fileInputRef.current && fileInputRef.current.click()} style={{ background: "transparent", border: "none", color: "#2563eb", fontSize: 13 }}>
                Import CSV/JSON
              </button>
              ...
              <input
                ...
                onChange={(ev) => {
                  ...
                  if (!f) return;
                  if (f.name.toLowerCase().endsWith(".csv")) importCSVFile(f);
                  else if (f.name.toLowerCase().endsWith(".json")) importJSONFile(f);
                  ev.target.value = "";
                }}
              />
            </div>
          ...
          <div style={{ background: "#fff", padding: 16, borderRadius: 10, boxShadow: "0 6px 18px rgba(15,23,42,0.04)" }}>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
              ...
              <button
                ...
                style={{ padding: 8, borderRadius: 8, border: "1px solid #e2e8f0", background: "#f1f5f9" }}
              >
                This Month
              </button>
            </div>
                ...
                <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                  <svg viewBox="0 0 160 160" width="120" height="120" aria-hidden>
                    {(() => {
                      ...
                      let start = -Math.PI / 2;
                      let i = 0;
                      const colorPalette = ["#60a5fa", "#34d399", "#fca5a5", "#fde68a", "#c7b3ff", "#93c5fd", "#fbbf24"];
                      return donutData.map((d) => {
                        ...
                        i++;
                        start = end;
                        return <path key={d.label} d={path} fill={fillColor} stroke="#fff" strokeWidth="1" />;
                      });
                    })()}
        ...
        <div style={{ marginTop: 18, background: "#fff", padding: 12, borderRadius: 10, boxShadow: "0 6px 18px rgba(15,23,42,0.04)" }}>
          ...
          {filtered.length === 0 ? (
            <div style={{ padding: 20, color: "#94a3b8" }}>No transactions found.</div>
          ) : (
                  ...
                  {filtered.map((tx) => (
                    <tr key={tx.id} style={{ borderTop: "1px solid #eef2f7" }}>
                      ...
                      <td style={{ ...td, textAlign: "center" }}>
                        <button onClick={() => deleteTransaction(tx.id)} style={{ background: "transparent", border: "none", color: "#ef4444" }}>
                          ✕
                        </button>
                      </td>
                    </tr>
